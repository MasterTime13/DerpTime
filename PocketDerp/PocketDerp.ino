...
==========================================================
Функции:
Лежачая поза Дерпи, пони моргает, комната, режим ночи, возможность кормить
маффинами, возможность гладить, возможность спать. Внешний таймер голода
(НАКОНЕЦ-ТО!), встроенный таймер голода, отключение голода, отключение побега.
Если Дерпи долго "дразнить" маффином, она начнёт что-то подозревать)).
Если Дерпи гладить, она довольно щурится)).
Шкала голода, настройки, Дерпи убегает (когда голодает), возможность
перезапуска. Добавлены короткие фразочки :3
*/
...
// Контакты "A" "B" "C", соответственно с схемой (можно заменить на другие
// цифровые)

#define soundPIN 35
//Контакт "D" (на схеме) - Пьезоэлемент или динамик для вывода звука. (можно
//заменить на другой цифровой)

#define GOLODOVKA 2
// сколько едениц голода отнимается в час (при использовании внешнего таймера,
// на базе часов реального времени)
...
static unsigned char wallpaper[]{
    0x00, 0x04, 0x08, 0x04, 0x00, 0x80, 0x40, 0x80, 0x00, 0x04, 0x08,
    0x04, 0x00, 0x80, 0x40, 0x80, 0x00, 0x04, 0x08, 0x04, 0x00, 0x80,
    0x40, 0x80, 0x00, 0x04, 0x08, 0x04, 0x00, 0x80, 0x40, 0x80, 0x00,
    0x04, 0x08, 0x04, 0x00, 0x80, 0x40, 0x80, 0x00, 0xFF, 0xFF, 0x00,
...
void draw() {
  ...
  u8g.drawPixel(DERPxH - 2, DERPyH - 7);
  u8g.drawLine(DERPxH + 2, DERPyH - 12 + glaza, DERPxH + 5,
               DERPyH - 12 + glaza);
  u8g.drawLine(DERPxH - 2, DERPyH - 12 + glaza, DERPxH - 4,
               DERPyH - 12 + glaza);
  ...
  case 1:
    ...
    if (select13item == 1) {
      ...
      if (muffinX >= DERPxH - 10 && muffinY >= DERPyH - 15 &&
          muffinX <= DERPxH + 10 && muffinY <= DERPyH + 2 && SLEEP < 3) {
        muffinX = 7;
        muffinY = 7;
        direct1 = 1;
        select13item = 0;
